...
def do_something(AvailNode_List):
    print("do something")
    FrameDrop = framedrop("/home/share/nfs/traffic-mini.mp4", "/home/share/nfs/result-mini.mp4", 'CORREL', 85,
                          AvailNode_List)
...
def home():
    videoPath = requests.args.get('fimeName')
    ...
    AvailNode_List = [
        node.Node(ob['name'], ob['cpu_usage'], ob['total_cpu'], ob['memory_usage'], ob['total_memory'], ob['isgpu'],
                  ob['FLOPS']) for ob in json_data2]
